...
import { getById, update as updateOrder } from '../services/order';
...
export default function PaymentReturn({ setTitle }) {
  ...
  useEffect(() => {
    if (params) getById(params.vnp_TxnRef).then((res) => setOrder(res));
  }, [params]);
        ...
        <div className='flex justify-center items-start min-h-[50vh]'>
          <div className='max-w-md bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 p-4 sm:p-8 min-w-[50vw]'>
            <h1 className='text-xl text-center font-bold'>
              {codeResponseVnPay[params?.vnp_ResponseCode] || 'Thông Tin Đơn Hàng'}
            </h1>
            <table>
              <thead></thead>
              <tbody>
            ...
            <Link to='/products' className='text-center block py-2 px-8 bg-black hover:bg-black/70 text-white rounded' >
              Tiếp tục mua sắm
            </Link>
          </div>
        </div>
